  ...
  return (
    <div className="scene" aria-hidden="true">
      <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" className="scene-svg">
        <defs>
          {/* 🔴 v0.37 — GÖKYÜZÜ ARTIK ÇİZİLMİYOR, PERDEYE DÖNÜŞTÜ.
              Eski hâlde bu gradyan OPAK bir lacivert gökyüzü boyuyordu
              (#070B16 → #16203A, ton ~222°) ve sayfanın arkasındaki
              fotoğrafı tamamen örtüyordu. İki arka plan üst üste
              gelince ikisi de kaybolur.

              Artık aynı gradyan yalnız KOYULUK katıyor: üstte yoğun
              (menü okunsun), aşağı doğru açılıyor (fotoğraf yaşasın).
              Renkler paletten geliyor — `var(--bg)` ile bağlı, yani
              app'in koyu teması değişirse sahne de değişir.

              🆕 SINIF: "İKİ ARKA PLANI ÜST ÜSTE KOYAMAZSIN — BİRİ
              ZEMİN OLUR, ÖTEKİ PERDE." */}
          <linearGradient id="sky" x1="0" y1="0" x2="0" y2="1">
            {/* 🔴 SAYFA PERDESİ ZATEN %76. Buraya bir %92 daha koyunca
                iki perde çarpıştı ve fotoğraf tamamen kayboldu —
                çizdirip görene kadar fark etmedim. Bu katman artık
                yalnız MENÜ ŞERİDİNİ koyulaştırıyor (üstte %38) ve
                aşağı doğru sıfıra iniyor. */}
            <stop offset="0%" stopColor="var(--bg)" stopOpacity="0.38" />
            <stop offset="34%" stopColor="var(--bg)" stopOpacity="0.10" />
            <stop offset="72%" stopColor="var(--bg)" stopOpacity="0" />
            <stop offset="100%" stopColor="var(--bg)" stopOpacity="0.16" />
          </linearGradient>
          <radialGradient id="dawn" cx="78%" cy="74%" r="55%">
            <stop offset="0%" stopColor="var(--gold)" stopOpacity="0.30" />
            <stop offset="55%" stopColor="var(--goldDeep)" stopOpacity="0.12" />
            <stop offset="100%" stopColor="var(--bg)" stopOpacity="0" />
          </radialGradient>
          {/* Kanat: fotoğrafın üstünde SİLUET. Opak kalıyor ki form
              okunsun; tonu sıcak nötrlerden. */}
          <linearGradient id="wing" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="var(--bg)" stopOpacity="0.97" />
            <stop offset="70%" stopColor="var(--bgAlt)" stopOpacity="0.92" />
            <stop offset="100%" stopColor="var(--bgAlt)" stopOpacity="0.80" />
          </linearGradient>
          <radialGradient id="tipglow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="var(--goldText)" stopOpacity="0.95" />
