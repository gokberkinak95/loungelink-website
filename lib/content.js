...
export const SITE = {
  name: "LoungeLink",
  // ══════════════════════════════════════════════════════════════════
  // 🔴 30 AĞUSTOS · v0.44 — LOUNGE SURF'ÜN TONU, BİZİM CÜMLELERİMİZ.
  //
  // Gökberk lounge.surf'ü örnek gösterdi: "kullanılan dil ve tonu
  // bence çok daha iyi." Ölçtüm — haklı ve sebebi biçimsel değil
  // YAPISAL. Onların her başlığı bir ZITLIK kurar:
  //     "Spend the hours between flights with someone instead of a screen."
  //     "You've already mastered the miles. Now master the connection."
  //     "Connecting flights, connecting travelers."
  //     "Type your flight, get matched."
  //
  // Bizim eski kahramanımız ("Senin uçağında, lounge'da yeri olan biri
  // var.") bir DURUM bildiriyordu. Doğru ve sıcak, ama okuyucuya bir
  // şey YAPTIRMIYOR: karşısına bir seçenek koymuyor.
  //
  // Zıtlık formülü tam da onu yapıyor: iki hâli yan yana koyup
  // okuyucuyu birini seçmeye davet ediyor. Ve "instead of a screen"
  // gibi somut bir düşman veriyor — soyut bir "yalnızlık" değil,
  // elindeki telefon.
  //
  // 🆕 SINIF: "BİR BAŞLIK DURUM BİLDİRİYORSA OKUNUR; İKİ HÂLİ YAN YANA
  // KOYUYORSA SEÇTİRİR — VE PAZARLAMA METNİNİN İŞİ OKUTMAK DEĞİL
  // SEÇTİRMEKTİR."
  //
  // ⚠️ TAKLİT DEĞİL, FORMÜL. Onların cümlelerini çevirmedim; kalıbı
  // aldım ve BİZİM ürünümüzün gerçeğiyle doldurdum. Bizim farkımız
  // kural motoru — ve o, onların hiçbir cümlesinde yok.
  // ══════════════════════════════════════════════════════════════════
  hero: "Aktarmadaki üç saati bir ekranla değil, biriyle geçir.",
  heroLead: "Aktarmadaki üç saati",
  heroEm: "bir ekranla değil, biriyle geçir.",
  heroEyebrow: "Salon hakkı cüzdanı",
  // Ritim + zıtlık. Bu cümle sitede iki yerde geçiyordu (kahraman ve
  // footer); artık yalnız footer'da — aynı cümleyi iki kez basmak
  // vurgu değil, tekrar.
  heroBaglanti: "Aktarma uçuşları bağlar. Biz yolcuları.",
  // Alt metin TEK CÜMLE + tek nefeslik ikinci cümle. Eskisi 47
  // kelimeydi ve dört cümleye bölünmüştü; Lounge Surf'ün alt metni
  // ürünün ne yaptığını TEK cümlede söylüyor. Uzun anlatım aşağıda
  // zaten var — ilk ekranın işi ikna etmek değil, yönlendirmek.
  heroSub:
    "LoungeLink, aynı uçuşta olan iki yolcuyu kalkıştan önce aynı salonda " +
    "buluşturur: birinin kartında kullanılmayan bir misafir hakkı vardır, " +
    "öbürünün yoktur. Kapıda alınıp alınmayacağını, sen başvurmadan " +
    "kural motoru söyler.",
  // "You've already mastered the miles" kalıbı — kart sahibine.
  tagline: "Milleri zaten topladın. Sıra o kartın yanındaki koltukta.",
  // "See your lounges. Then see who's in them." kalıbının bizdeki hâli.
  introKisa: "Önce salonu gör, sonra içindeki insanı.",
  intro:
...
export const FLOW = [
  // 🔴 v0.44 — ÜÇ ADIM ARTIK ÜÇ FİİL.
  // Lounge Surf: "Post your trip → Wait for a match → Meet in the
  // lounge." Üç kelime, üç fiil, tek nefes. Bizimkiler doğruydu ama
  // ilki 28, ikincisi 46 kelimeydi — "ne kadar kolay" diyen bir bölüm,
  // okunması en zor bölümdü.
  //
  // 🆕 SINIF: "BASİTLİĞİ ANLATAN BİR BÖLÜM UZUNSA, ANLATTIĞI ŞEYİ
  // KENDİ BİÇİMİYLE YALANLAR."
  //
  // Detay silinmedi, YERİ DEĞİŞTİ: kural şartları aşağıdaki kural
  // motoru bölümünde, kredi iadesi güven bölümünde duruyor.
  { n: "01", t: "Uçuşunu yaz",
    d: "Havalimanı ve tarih yeter. Uçuş numarası isteğe bağlı — girmeyeni dışarıda bırakmayız." },
  { n: "02", t: "Eşleşmeni bekle",
    d: "Aynı uçuştakiler en üstte. Her kartta kural rozeti: kim seni gerçekten içeri alabilir, başvurmadan görürsün." },
  { n: "03", t: "Salonda buluş",
    d: "Giriş buluşmadan önce teyit edilir. Kapıda alınmazsan kredin iade edilir. Gerisi iki yolcunun sohbeti." },
];
...
export const SHELF = [
  ...
  // ══════════════════════════════════════════════════════════════════
  // 🔴 19 EYLÜL — ÜÇ YAZI, GÖRDÜĞÜ KAREYE UYDURULDU.
  //
  // Raftaki üç kare (`eslesme`, `oturum`, `puanla`) üründe ARTIK AYRI
  // EKRAN OLARAK VAR OLMAYAN üç şeye işaret ediyordu. Görseller v5.12.0
  // sahnelerinden tazelenince yazılar karelere yalan söylemeye başladı.
  //
  // Üç seçenek vardı: yazıları uydurmak, kareleri raftan çıkarmak, ya da
  // üç yeni ekran icat etmek. Üçüncüsü ürünün olmadığı bir şeyi vaat
  // etmek olurdu; ikincisi rafı 7'den 4'e düşürüp GERÇEKTEN VAR OLAN üç
  // anı anlatmaktan vazgeçmek olurdu. Kalan tek dürüst yol birincisiydi:
  // her kare gerçek, her yazı o karede GÖRÜNEN şeyi anlatıyor.
  //
  // 🆕 SINIF: "BİR VİTRİN YAZISI, ARKASINDAKİ KAREDEN ÖNCE YAZILDIYSA,
  // ÜRÜN DEĞİŞTİĞİNDE İLK YALAN SÖYLEYEN O OLUR."
  // ══════════════════════════════════════════════════════════════════
  { src: "/screens/ss-eslesme.jpg", tag: "Yol arkadaşı",
    soz: "Salon kapısının ötesi: karşılıklı onaylanan her bağlantı kendi sohbetini açar — oturum olmasa da.",
    alt: "Tanış sohbeti — karşılıklı bağlantı kurulunca açılır", w: 480, h: 1039 },
  ...
  { src: "/screens/ss-oturum.jpg", tag: "Oturum",
    soz: "Konumun paylaşılmaz. Yalnız seçtiğin durum gider — ve ne zaman duracağına sen karar verirsin.",
    alt: "Oturum · canlı durum paylaşımı", w: 480, h: 1039 },
  { src: "/screens/ss-puanla.jpg", tag: "Puan",
    soz: "Oturum biter, iki taraf da puanlar. İzin verirsen yorumun sitede görünür; vermezsen yalnız bizde kalır.",
    alt: "Değerlendirme — biten oturumun geri bildirimi", w: 480, h: 1039 },
...
export const SHOTS_TRUST = [
  // v0.66 — `ss-baglanti.jpg` çıkarıldı: tek satırlık liste, ekranın üçte ikisi
  // boş; 150px'te "bozuk görsel" gibi okunuyordu. Üç kare, üçü de dolu.
  ...
  // 🔴 21 EYLÜL — `ss-koyu.jpg` BURADAN ÇIKARILDI (DONMUŞ KARE).
  // O görselin kaynağı `10_kesfet_koyu` sahnesiydi ve 30 Ağustos'ta
  // ÜRETİLMEYİ BIRAKTI (tek tema kaldı). `site_ekran_tazele.py`nin
  // eşleme tablosu güncellendi, ama BU LİSTE güncellenmedi: dosya
  // diskte kaldığı için site onu basmaya devam etti. Yani SURUM.json'ın
  // yönettiği 14 karenin dışında, hiçbir turda tazelenmeyen 15. bir
  // kare vardı — ve ziyaretçinin gördüğü tek eski ekran oydu.
  // Yerine `ss-bildirim.jpg`: yönetilen listede, güncel, ve o ana
  // kadar sitede HİÇ kullanılmıyordu.
  // 🆕 SINIF: "BİR GÖRSELİ TAZELEYEN TABLODAN ÇIKARIRSAN SİLİNMEZ,
  // DONAR — VE DONMUŞ KARE, EKSİK KAREDEN DAHA UZUN YAŞAR."
  { src: "/screens/ss-bildirim.jpg", alt: "Bildirimler — akış tek yerde", w: 480, h: 1039 },
];
...
// ══════════════════════════════════════════════════════════════════
// 🔴 30 AĞUSTOS · v0.44 — BÖLÜM BAŞLIKLARI ARTIK BURADA.
//
// Bu dosyanın kendi başlığı "TÜM METİNLER TEK DOSYADA" diyordu.
// Ölçtüm: ana sayfadaki 13 `h2`den 8'i JSX'in içine gömülü DÜZ YAZIydı.
// Yani kural yazılmış, uygulanmamıştı — ve her yeni bölüm kuralı bir
// kez daha deliyordu.
//
// 🆕 SINIF: "BİR KURALI DOSYANIN BAŞINA YAZMAK ONU UYGULAMAK DEĞİLDİR;
// UYGULANMAYAN KURAL, İHLALİ MEŞRULAŞTIRAN BİR SÜSTÜR."
//
// BAŞLIKLAR LOUNGE SURF KALIPLARIYLA YENİDEN YAZILDI:
//   · zıtlık        → "X yerine Y"
//   · sahiplik      → "X'e zaten sahipsin, şimdi Y"
//   · ritim/karşıtlık→ "A'yı bağlar. Biz B'yi."
//   · kısa eylem    → "Uçuşunu yaz, eşleş."
// Her başlık kullanıcının ELİNDEKİNDEN başlar, üründen değil.
// ══════════════════════════════════════════════════════════════════
export const BOLUM = {
  akis:   { eyebrow: "Nasıl çalışır",
            h2: "Uçuşunu yaz, eşleş.",
            // 4 Eylül — "3 saniyede" vaadi vitrinden kalktı (süre vaadi verilmez);
            // çağrı kural bölümünün cümlesine bağlanıyor: "Kalkıştan önce bilinir."
            beat: "Kalkıştan önce kuralı öğren" },
  cuzdan: { eyebrow: "Önce senin kartın",
            h2: "Kartında kaç kapı var? Üye olmadan öğren.",
            beat: "Kartını seç, cevabını oku" },
  neden:  { eyebrow: "Neden LoungeLink",
            h2: "Aynı uçakta iki kişi: birinin yeri var, öbürünün yok." },
  kural:  { eyebrow: "Kural motoru",
            h2: "Kapıda öğrenilmez. Kalkıştan önce bilinir.",
            beat: "Kartını seç, kuralı oku" },
  kapsam: { eyebrow: "Kapsam",
            h2: "Önce salonu gör, sonra içindeki insanı." },
  guven:  { eyebrow: "Güven ve gizlilik",
            h2: "Kimlik doğrulanır, puan karşılıklıdır, veri satılmaz." },
  host:   { eyebrow: "Kapıyı açan taraf",
            h2: "Milleri zaten topladın. Sıra o kartın yanındaki koltukta.",
            statement: "Bu yıl kartındaki misafir haklarından kaçını kullandın?" },
  plan:   { eyebrow: "Abonelik",
            h2: "Ağırlarsan ödemezsin." },
  sss:    { eyebrow: "Sık sorulanlar",
            h2: "Aklındaki soru muhtemelen burada." },
};

// 🔴 MİKRO SAHNE — Lounge Surf'ün en iyi hamlesi bu: ürünü anlatmak
// yerine bir ANI gösteriyor ("I'm at gate B12, blue jacket"). Bir
// cümlelik somut sahne, üç paragraflık özellik listesinden daha çok
// şey anlatır çünkü okuyucu kendini oraya koyar.
//
// 🆕 SINIF: "SOMUT BİR CÜMLE, DOĞRU BİR PARAGRAFTAN DAHA İKNA
// EDİCİDİR — İKNA BİLGİYLE DEĞİL, CANLANDIRMAYLA OLUR."
export const SAHNE = [
  { kim: "Deniz", saat: "13:12", m: "Uçuşun aynı, güzel. Salona 13:50 gibi geçmeyi düşünüyorum." },
  { kim: "sen",   saat: "13:14", m: "Ben güvenlikten yeni çıktım. 13:50 bana da uyar." },
  { kim: "Deniz", saat: "13:15", m: "Kapının solundaki kitapçının önünde buluşalım mı? Lacivert ceket." },
];
...
export const HOST_RISK = [
  ...
  { q: "Ya gelmezse?", a: "Oturum iki taraflı onayla başlar. Sen başlattıktan 15 dakika sonra karşı taraf hâlâ yoksa uygulamadan “Gelmedi” dersin: buluşma o an kapanır, misafirsen kredin iade edilir ve gelmeyen tarafın güven puanına yazılır. Yanlışlık olduysa karşı taraf itiraz edebilir." },
